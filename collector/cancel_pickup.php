...
try {
    ...
    if ($pickup_result->num_rows === 0) {
        throw new Exception("Pickup not found or not in cancellable state.");
    }

    $pickup = $pickup_result->fetch_assoc();
    $request_id = $pickup['request_id'];

    // 2. Update pickup status to 'cancelled' with timestamp
    $update_pickup = "UPDATE pickups SET status = 'cancelled', cancelled_at = NOW() WHERE id = ?";
    $stmt = $conn->prepare($update_pickup);
    $stmt->bind_param("i", $pickup_id);
    $stmt->execute();

    // 3. Update pickup_request status back to 'Pending'
    $update_request = "UPDATE pickup_requests SET status = 'Pending' WHERE id = ?";
    $stmt = $conn->prepare($update_request);
    $stmt->bind_param("i", $request_id);
    $stmt->execute();

    // 4. Update collector's status in users table to 'active'
    ...
    $_SESSION['message'] = "Pickup #$pickup_id cancelled successfully";
    $_SESSION['message_type'] = "success";
} catch (Exception $e) {
    $conn->rollback();
    $_SESSION['message'] = "Error cancelling pickup: " . $e->getMessage();
    $_SESSION['message_type'] = "danger";
}
